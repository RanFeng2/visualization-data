...
t5 = find(chl>=1.0);
chl_tot = chl;
chl_tot(t4) = 38.0 .* chl(t4).^0.425 ;
chl_tot(t5) = 40.2 .* chl(t5).^0.507 ;

%数据边界
real_id = ~isnan(sst)&~isnan(chl_tot)&~isnan(zeu);
e_ratio = nan(4320,2160);
e_ratio(real_id) = max(0.04,min(0.72,(-0.0081.*sst(real_id)+0.0806.*log(chl_tot(real_id)./zeu(real_id))+0.426)));%%[边界暂时没写]
...
title('POC【0.0806chl_{tot}计算】','fontsize',15)
xlabel('longitude');
ylabel('latitude')
